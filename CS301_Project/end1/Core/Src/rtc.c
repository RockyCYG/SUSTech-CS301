...
u8 RTC_Init(void)
{
	...
	if(HAL_RTCEx_BKUPRead(&hrtc,RTC_BKP_DR1)!=0X5050)//是否第一次配置
	{
		...
		HAL_UART_Transmit(&huart1, (uint8_t *)"FIRST TIME RTC\n", 15, HAL_MAX_DELAY);
	}
...
{		 
	...
	if(__HAL_RTC_ALARM_GET_FLAG(&hrtc,RTC_FLAG_SEC)!=RESET)  //闹钟中断
	{
		__HAL_RTC_ALARM_CLEAR_FLAG(&hrtc,RTC_FLAG_ALRAF); 	//清除闹钟中断
		RTC_Get();				//更新时间   
		//printf("ALARM A!\r\n");
	}	
	__HAL_RTC_ALARM_CLEAR_FLAG(&hrtc,RTC_FLAG_OW); 		//清除溢出
}
...
void RTC_LCD_ShowWeek(int x,int y)
{
  switch (calendar.week)
  {
  case 0:
    LCD_ShowString(x, y, 200, 16, 16, (u8 *)"Sunday   ");
    break;
  case 1:
    LCD_ShowString(x, y, 200, 16, 16, (u8 *)"Monday   ");
    break;
  case 2:
    LCD_ShowString(x, y, 200, 16, 16, (u8 *)"Tuesday  ");
    break;
  case 3:
    LCD_ShowString(x, y, 200, 16, 16, (u8 *)"Wednesday");
    break;
  case 4:
    LCD_ShowString(x, y, 200, 16, 16, (u8 *)"Thursday ");
    break;
  case 5:
    LCD_ShowString(x, y, 200, 16, 16, (u8 *)"Friday   ");
    break;
  case 6:
    LCD_ShowString(x, y, 200, 16, 16, (u8 *)"Saturday ");
    break;
  }
}
