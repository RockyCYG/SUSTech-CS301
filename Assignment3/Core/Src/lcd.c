...
//    while ((*p <= '~') && (*p >= ' '))    //判断是不是非法字符!
    while ((*p <= '~') && (*p > 0))    //判断是不是非法字符!
    {
        ...
        if (*p >= ' ') {
            ...
            LCD_ShowChinese(x, y, *p - 1, size, BLACK, WHITE, 0);
            x += size;
        } else if (*p < 20) {
            LCD_ShowChar(x, y, inclines[*p - 10], size, 0);
            x += size / 2;
        } else {
            LCD_ShowChar(x, y, variants[*p - 20], size, 0);
            x += size / 2;
        }
//        x += size / 2;
        p++;
...
void LCD_ShowMissingPicture(uint16_t x, uint16_t y, uint16_t mx, uint16_t my, uint16_t
column, uint16_t row, unsigned short *pic) {
    uint16_t m, h;
    for (h = 0 + my; h < BLOCK_SIZE + my; h++) //60
    {
        for (m = 0 + mx; m < BLOCK_SIZE + mx; m++) //180
        {
            LCD_Fast_DrawPoint(m, h, pic[h * column + m + x - mx]);
        }
    }
    LCD_DrawRectangle(mx, my, mx + BLOCK_SIZE, my + BLOCK_SIZE);
}
...
//backcolor：背景颜色
//mode:0,正常显示,1,叠加显示
void LCD_ShowChinese(u16 x, u16 y, u16 index, u8 size, u16 pointcolor, u16 backcolor, u8 mode) {
    u8 temp, t, t1;
    u16 y0 = y;
    u8 csize = (size / 8 + ((size % 8) ? 1 : 0)) * (size);    //一个汉字数据字节数，直接存就可比如size=24一个汉字是｛72数据｝，16是32
    for (t = 0; t < csize; t++) {
        temp = chinese[index][t];            //得到点阵数据
        for (t1 = 0; t1 < 8; t1++) {
...
void LCD_ShowInclineChar(u16 x, u16 y, u16 index, u8 size, u16 pointcolor, u16 backcolor, u8 mode) {
    u8 temp, t, t1;
    u16 y0 = y;
    u8 csize = (size / 8 + ((size % 8) ? 1 : 0)) * (size / 2);    //一个汉字数据字节数，直接存就可比如size=24一个汉字是｛72数据｝，16是32
    for (t = 0; t < csize; t++) {
        temp = incline_characters[index][t];            //得到点阵数据
        for (t1 = 0; t1 < 8; t1++) {
            if (temp & 0x80)LCD_Fast_DrawPoint(x, y, POINT_COLOR);
            else if (mode == 0)
                LCD_Fast_DrawPoint(x, y, BACK_COLOR);
...
void LCD_ShowVariantChar(u16 x, u16 y, u16 index, u8 size, u16 pointcolor, u16 backcolor, u8 mode) {
    u8 temp, t, t1;
    u16 y0 = y;
    u8 csize = (size / 8 + ((size % 8) ? 1 : 0)) * (size / 2);    //一个汉字数据字节数，直接存就可比如size=24一个汉字是｛72数据｝，16是32
    for (t = 0; t < csize; t++) {
        temp = variant_characters[index][t];            //得到点阵数据
        for (t1 = 0; t1 < 8; t1++) {
            if (temp & 0x80)LCD_Fast_DrawPoint(x, y, POINT_COLOR);
            else if (mode == 0)
                LCD_Fast_DrawPoint(x, y, BACK_COLOR);
